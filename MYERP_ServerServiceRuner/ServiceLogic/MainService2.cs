...
Where isNull(a.Type,'') = 'N' And a.CheckDate is Null And a.InputDate > '2016-10-01' And a.InputDate > @BTime"; //已退过版纸未审核
				...
				try
				{
                    ...
                    MyConfig.MailAddress mAddress = MyConfig.GetMailAddress("IPQCAndScrapBack");
...
";
		...
		{
			MyRecord.Say("发送OQC检验报告..........");
			Thread t = new Thread(SendOQCEmail);
			t.IsBackground = true;
			t.Start();
			MyRecord.Say("发送昨日OQC检验报告.......");
		}
...
");
				string gd = @"
<DIV><FONT size=5 color=#ff0000 face=PMingLiU>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 昨日（{0:yy/MM/dd HH}点~{1:yy/MM/dd HH}点）OQC检验状况。</FONT></DIV>
<DIV><FONT size=5 face=PMingLiU>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<TABLE style=""BORDER-COLLAPSE: collapse"" cellSpacing=0 cellPadding=0 width=""100%"" border=0>
  <TBODY>
	<TR>
...
<DIV><FONT size=5 color=#ff33ff face=PMingLiU>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 详细批退内容请参考附档</FONT></DIV>
";
				string sd = @"
<DIV><FONT size=5 color=#ff0000 face=PMingLiU>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; （{0:yy/MM/dd HH}点~{1:yy/MM/dd HH}点）没有安排出货。</FONT></DIV>
";
				string br = @"
	<TR>
	<TD class=xl63 style=""BORDER-TOP: windowtext 0.5pt solid; BORDER-RIGHT: windowtext 0.5pt solid; BORDER-BOTTOM: windowtext 0.5pt solid; BORDER-LEFT: windowtext 0.5pt solid; BACKGROUND-COLOR: transparent"" align=center >
...
";
				...
				string SQLRS = @"
SELECT [a].[InspectDate],[a].[RGUID],[b].[DepartmentCode],[b].[CRMAMI],[a].[Numb],[a].[Result],[b].[RdsNo],a.[ProductCode],c.Name as ProductName
  FROM [_QC_OQC_Inspect_Memo] [a] LEFT OUTER JOIN [_QC_OQC_Inspect_Issues] [b] ON [b].[CGUID] = [a].[CGUID] AND [b].[RGUID] = [a].[RGUID]
								  LEFT OUTER JOIN [pbProduct] [c] ON [c].[code] = [a].[ProductCode]
 WHERE [a].[InspectDate] BETWEEN DATEADD(HOUR,8,CAST(CONVERT(Varchar(10),DATEADD(DAY,-1,GETDATE()),121) AS DateTime)) AND DATEADD(HOUR,8,CAST(CONVERT(Varchar(10),GETDATE(),121) AS DateTime))
";   //OQC
				string SQLQS = @"
 SELECT [a].[SalesMan], [a].[Assistant], [a].[SendAddress], [a].[Bonded], [a].[Secret], [b].[Numb] AS [ShipmentAdviceNumb], [b].[ReceiptDate],
	   [c].[typename] AS [TypeName], [c].[type], [c].[name] AS [ProductName], [c].[unit], [c].[PhoneSubject], [c].[Description], [c].[secret] AS [ProductSecret],
	   [c].[Bonded] AS [ProductBonded], [c].[Specs2] AS [Size], [c].[CustCode] AS [ProductCustCode], [c].[DeclaretionName], [c].[mCustNO] AS [CustProdCode],
	   [c].[financeType], [c].[isExpenses], [c].[NumberInteger], [d].[name] AS [CustomerName], [d].[moneytype], [e].[custorderid] AS [CustOrderNo],
	   [a].[CloseDate], [a].[OQC], [a].[OQCRdsNo], [a].[OQCDate], [b].[EndDate], [b].[ShippingNumber], [f].[Name] AS [DepartmentName], 
	   [x].[id], [x].[zbid], [x].[InspectDate], [x].[CustCode],[x].[CRMAMI],
	   [x].[ProductCode], [x].[OrderRdsNo], [x].[ShipmentAdviceRdsNo], [x].[MID], [x].[PID], [x].[Numb], [x].[Numb1], [x].[RdsNo], [x].[IssuseMemo],
	   [x].[DepartmentCode], [x].[SolutionMemo], [x].[ConfirmSignDate], [x].[ConfirmSigner], [x].[Remark], [x].[CGUID], [x].[RGUID]
  FROM [_QC_OQC_Inspect_Issues] [x]
	   INNER JOIN [_SD_ShipmentAdvice] [a] ON [x].[ShipmentAdviceRdsNo] = [a].[RdsNo]
	   INNER JOIN [_SD_ShipmentAdvice_List] [b] ON [b].[zbid] = [a].[_id] AND [b].[_id] = [x].[PID]
	   INNER JOIN [pbProduct] [c] ON [c].[code] = [x].[ProductCode]
	   INNER JOIN [coCustomer] [d] ON [d].[code] = [x].[CustCode]
	   INNER JOIN [coOrder] [e] ON [e].[rdsno] = [b].[OrderRdsNo]
	   LEFT OUTER JOIN [pbDept] [f] ON [f].[code] = [x].[DepartmentCode]
WHERE [x].[InspectDate] BETWEEN DATEADD(HOUR,8,CAST(CONVERT(Varchar(10),DATEADD(DAY,-1,GETDATE()),121) AS DateTime)) AND DATEADD(HOUR,8,CAST(CONVERT(Varchar(10),GETDATE(),121) AS DateTime))
";
				DateTime NowTime = DateTime.Now;
				string brs = "";
				MyRecord.Say("后台计算-OQC");
				MyData.MyDataTable mrs = new MyData.MyDataTable(SQLRS);   //OQC
				MyData.MyDataTable mrq = new MyData.MyDataTable(SQLQS);   //QS
				MyRecord.Say("后台计算完成。");
				try
				{
					HSSFWorkbook wb = new HSSFWorkbook();
					if (mrq.MyRows.IsNotEmptySet())  //批退
					{
						MyRecord.Say(string.Format("OQC批退，找到了：{0} 行。", mrq.MyRows.Count));
						string[] fNames = new string[] { "RdsNo", "InspectDate", "CustCode", "ProductCode", "ProductName", "CustProdCode", "OrderRdsNo", "ShipmentAdviceRdsNo", "Numb", "Numb1", "IssuseMemo", "DepartmentName", "CRMAMI", "SolutionMemo", "ConfirmSignDate", "ConfirmSigner", "Remark" };
						string[] fTitles = new string[] { "出货检验记录单号", "抽检时间", "客户", "产品编号", "产品料号", "客户料号", "订单编号", "出货通知单", "出货数量", "检验(箱)数", "不良现象", "责任部门", "重/主/次缺", "处理方案", "处理时间", "处理人", "备注" };
						string[] fSortFields = new string[] { "RdsNo", "InspectDate","CustCode" };
						ExportToExcelIPQCAndScrapBack(wb, mrq, fNames, fTitles, "OQC批退表", fSortFields);
					}
					else
					{
						MyRecord.Say("没有未退料");
					...
					string xBodyString = string.Empty;
					if (mrs.MyRows.IsNotEmptySet())
					{
						double b = mrs.MyRows.Count;
						var vb1 = from a in mrs.MyRows
								  where a.Value("RdsNo").IsNotEmpty() 
								  select a;
						double b1 = vb1.IsNotEmptySet() ? vb1.Count() : 0;

						var vCR = mrs.MyRows.Where(x => x.Value("CRMAMI") == "CR");
						double cr = vCR.IsNotEmptySet() ? vCR.Count() : 0;

						var vMA = mrs.MyRows.Where(x => x.Value("CRMAMI") == "MA");
						double ma = vMA.IsNotEmptySet() ? vMA.Count() : 0;

						var vMI = mrs.MyRows.Where(x => x.Value("CRMAMI") == "MI");
						double mi = vMI.IsNotEmptySet() ? vMI.Count() : 0;


						string ss1 = string.Format("CR:{0},MA:{1},MI:{2}", cr, ma, mi);

						brs += string.Format(br, b, b1, (b1 / b).ToString("0.000%"), ss1);
						string gds = string.Format(gd, NowTime.AddDays(-1).Date.AddHours(8), NowTime.Date.AddHours(8), brs);
						xBodyString = string.Format(body, gds, NowTime, MyBase.CompanyTitle);
						MyRecord.Say("邮件内容生成完毕。");
					}
					else
					{
						string sds = string.Format(sd, NowTime.AddDays(-1).Date.AddHours(8), NowTime.Date.AddHours(8));
						xBodyString = string.Format(body, sds, NowTime, MyBase.CompanyTitle);
						MyRecord.Say("无内容");
					}
					...
					wb.Close();
					sm.MailBodyText = MyConvert.ZH_TW(xBodyString);
					sm.Subject = MyConvert.ZH_TW(string.Format("{1}{0:yy年MM月dd日}_OQC检验日报(测试)", NowTime.AddDays(-1), MyBase.CompanyTitle));
                    //string mailto = ConfigurationManager.AppSettings["OQCMailTo"], mailcc = ConfigurationManager.AppSettings["OQCMailCC"];
                    //MyRecord.Say(string.Format("\r\n MailTO:{0} \r\n MailCC:{1}", mailto, mailcc));
                    //sm.MailTo = mailto; // "my18@my.imedia.com.tw,xang@my.imedia.com.tw,lghua@my.imedia.com.tw,my64@my.imedia.com.tw";
                    //sm.MailCC = mailcc; // "jane123@my.imedia.com.tw,lwy@my.imedia.com.tw,my80@my.imedia.com.tw";
                    ////sm.MailTo = "my80@my.imedia.com.tw";

                    MyConfig.MailAddress mAddress = MyConfig.GetMailAddress("OQC");
                    MyRecord.Say(string.Format("MailTO:{0}\r\nMailCC:{1}", mAddress.MailTo, mAddress.MailCC));
                    sm.MailTo = mAddress.MailTo;
                    sm.MailCC = mAddress.MailCC;
                    //sm.MailTo = "my80@my.imedia.com.tw";
