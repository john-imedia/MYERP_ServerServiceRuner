...
namespace MYERP_ServerServiceRuner
{
    public partial class MainService
    {
        ...
        int _PlanExtendTime = 100;

        private void Plan_LoadDataSource(DateTime NowTime, int classtype)
        {
            string SQLPlan = @"
SELECT [a].[RdsNo], [a].[Process], [a].[Department], [a].[PlanBegin], [a].[PlanEnd], [a].[Inputer], [a].[Checker], [b].[MachineCode], [b].[ReqNumb], [b].[EDD],
       [b].[BDD], [b].[reqtime], [b].[WLevel] AS [WorkLevel], [b].[CapF] AS [CapacityRate], [b].[PrepF] AS [PrepRate], [b].[Capacity], [b].[PrepTime],
       [b].[HRNumb], [b].[PartNo] AS [PartID], [b].[PRODNO], [b].[ProdCode] AS [ProductCode], [b].[REMARK], [b].[PN] AS [ColNumb], [b].[side],
       [b].[AutoDuplexPrint], [b].[CUSTCODE], [c].[PhoneSubject], [c].[name] AS [ProductName],[b].[AdjustTime],[b].[HrNumb],[b].[ProdNo],[b].[Remark],[b].[Side],
	   [p].[name] AS [ProcessName],[m].[name] AS [MachineName],[d].[name] AS [DepartmentName],[d].[FullSortID],[m].[FinishMeasurement],[m].[StaticMeasurement],
       (CASE WHEN [b].[ProcNo] = '2000' THEN (SELECT TOP(1) CASE WHEN ([mp].[ca1] + [mp].[ca2]) > 0
                                                               AND ([mp].[cb1] + [mp].[cb2]) > 0 THEN 2 ELSE 1 END
                                                FROM [dbo].[moProdProcedure] AS [mp]
                                               WHERE [mp].[zbid] = [b].[PRODID]
                                                     AND [mp].[id] = [b].[ProcID] ORDER BY [mp].[id]) ELSE 1 END) AS [ProdSide]
  FROM [dbo].[_PMC_ProdPlan] [a]
       INNER JOIN [dbo].[_PMC_ProdPlan_List] [b] ON [a].[_ID] = [b].zbid
	   LEFT OUTER JOIN [dbo].[moProcedure] [p] ON [a].[Process] = [p].[code]
	   LEFT OUTER JOIN [dbo].[moMachine] [m] ON [b].[MachineCode] = [m].[code]
	   LEFT OUTER JOIN [dbo].[pbDept] [d] ON [a].[DepartmentID] = [d].[_ID]
       LEFT OUTER JOIN [dbo].[pbProduct] AS [c] ON [b].[ProdCode] = [c].[code]
 WHERE [a].[Status] >= 2
       AND ISNULL([b].YieldOff, 0) = 0
       AND [a].[ClassType] = @ClassType
       AND DATEDIFF(DAY, [a].[PlanBegin], @PlanDate) = 0
       AND [b].EDD < DATEADD(mi, @PlanExtendTime, [a].PlanEnd)
       AND [b].BDD > '2000-01-01 00:00:00'
 ORDER BY [d].[FullSortID], [a].[Process], [b].[MachineCode], [b].[_ID];
";
            string SQLFinish = @"
 SELECT [a].[ProcessID] AS [ProcessCode], [a].[Numb1], [a].[MachinID] AS [MachineCode], [a].[StartTime], [a].[EndTime], [a].[ProduceNo], [a].[InputDate],
       [a].[Inputer], [a].[Numb2], [a].[Operator], [a].[Remark], [a].[CustCode], [a].[PartID], [a].[Remark2], [a].[ColNumb], [a].[NAColNumb], [a].[AdjustNumb],
       [a].[SampleNumb], [a].[Rejector], [a].[RejectDate], [a].[PaperColNumb], [a].[ProductCode], [a].[AccNoteRdsNo], [a].[Side], [a].[AutoDuplexPrint],
       [a].[DestroySubLevelItemCode], [c].[PhoneSubject] AS [PhoneSubject], [c].[name] AS [ProductName]
  FROM [dbo].[ProdDailyReport] AS [a]
       INNER JOIN [dbo].[AllMaterialView] [c] ON [a].[ProductCode] = [c].[code]
 WHERE [a].[RptDate] BETWEEN @DepartmentPlanFinishDateBegin AND @DepartmentPlanFinishDateEnd
";
            DateTime DepartmentPlanFinishDateBegin, DepartmentPlanFinishDateEnd;

            if (classtype == 1) //白班
            {
                DepartmentPlanFinishDateBegin = NowTime.Date.AddHours(6).AddMinutes(55);
                DepartmentPlanFinishDateEnd = NowTime.Date.AddDays(3).AddHours(19).AddMinutes(55);
            }
            else  //夜班
            {
                DepartmentPlanFinishDateBegin = NowTime.Date.AddHours(18).AddMinutes(55);
                DepartmentPlanFinishDateEnd = NowTime.Date.AddDays(4).AddHours(7).AddMinutes(55);
            }
            MyData.MyDataParameter[] mps = new MyData.MyDataParameter[]
			{
				new MyData.MyDataParameter("@PlanDate", NowTime, MyData.MyDataParameter.MyDataType.DateTime),
				new MyData.MyDataParameter("@ClassType", classtype, MyData.MyDataParameter.MyDataType.Int),
				new MyData.MyDataParameter("@PlanExtendTime",_PlanExtendTime , MyData.MyDataParameter.MyDataType.Int)
			};
            MyData.MyDataParameter[] mpfs = new MyData.MyDataParameter[]
			{
				new MyData.MyDataParameter("@DepartmentPlanFinishDateBegin", DepartmentPlanFinishDateBegin, MyData.MyDataParameter.MyDataType.DateTime),
				new MyData.MyDataParameter("@DepartmentPlanFinishDateEnd", DepartmentPlanFinishDateEnd, MyData.MyDataParameter.MyDataType.DateTime),
			};
            DateTime _StartTime = DateTime.Now;
            MyRecord.Say(string.Format("3.1.1排程{0:yy.MM.dd}{1}，完工单起：{2:yy.MM.dd HH:mm}止：{3:yy.MM.dd HH:mm}。", NowTime, classtype == 1 ? LCStr("白班") : LCStr("夜班"), DepartmentPlanFinishDateBegin, DepartmentPlanFinishDateEnd));
            MyRecord.Say(string.Format("3.1.2正在读取生产计划。AT:{0:yy.MM.dd HH:mm:ss}", DateTime.Now));
            MyData.MyDataTable md1 = new MyData.MyDataTable(SQLPlan, 100, mps);
            ...
            MyData.MyDataTable md2 = new MyData.MyDataTable(SQLFinish, 100, mpfs);
        ...
        class Plan_GridItem
        {
            public Plan_GridItem(IGrouping<object, Plan_Item> g, int classtype)
            {
                /*
                if (g.Count() > 0)
                {
                ...
                if (g.Count() > 0)
                {
                    ...
                    Checker = curItem.Checker;
                    var vd = from a in g
                             where (a.Bdd - a.PlanBegin).TotalMinutes > -10 && (a.Edd - a.PlanEnd).TotalMinutes < 10
                             select a;
                    if (vd != null && vd.Count() > 0)
                    {
                        BDD = vd.Min(x => x.Bdd);
                        EDD = vd.Max(x => x.Edd);
                        StdFinishTimes = vd.Sum(x => x.StdFinishTimes);

                        DateTime ThisTime = DateTime.Now;
                        TimeSpan xSpan = TimeSpan.FromHours(2);
                        if (classtype == 1)
                        {
                            if (ThisTime.Hour >= 13 && ThisTime.Hour < 15)
                            {
                                xSpan = TimeSpan.FromHours(3);
                            }
                            else if (ThisTime.Hour >= 18 && ThisTime.Hour < 20)
                            {
                                xSpan = TimeSpan.FromHours(3);
                            }
                        }
                        ...
                        {
                            if (ThisTime.Hour >= 1 && ThisTime.Hour < 3)
                            {
                                xSpan = TimeSpan.FromHours(3);
                            }
                            else if (ThisTime.Hour >= 6 && ThisTime.Hour < 8)
                            {
                                xSpan = TimeSpan.FromHours(3);
                            }
                        }
                        DateTime StandTime = DateTime.Now.Subtract(xSpan);
                        var vBeginTime = from a in g
                                         where a.Bdd < StandTime
                                         select a.Bdd;
                        if (vBeginTime.IsNotEmptySet())
                            StdCheckDateTime = vBeginTime.Max();
                        else
                            StdCheckDateTime = StandTime;
            ...
            /// 按排程完成的时数；
            /// </summary>
            public double StdFinishTimes { get; private set; }

            public DateTime StdFinishDateTime
            {
                get
                {
                    return BDD.AddHours(StdFinishTimes);
                }
            }

            public DateTime StdCheckDateTime { get; private set; }

            public string PlanFinishStatus
            {
                get
                {
                    return (StdCheckDateTime - StdFinishDateTime).TotalMinutes > 5 ? "red" : "green";
                }
            }
            ...
            public double Y1
            {
                get
                {
                    return PlanCount != 0 ? Convert.ToDouble(FinishCount) / Convert.ToDouble(PlanCount) : 0;
                }
            }
            ...
            public double Y2
            {
                get
                {
                    return FinishProdNumb != 0 ? Convert.ToDouble(FinishProdNumb) / Convert.ToDouble(PlanProdNumb) : 0;
                }
            }
        }
        ...
        class Plan_Item
        {
            ...
            /// 标准加工时间，含调机时间；
            /// </summary>
            public double StdWorkTime
            {
                get
                {
                    return (Edd - Bdd).TotalHours;
                }
            }
            ...
            public double Performance
            {
                get
                {
                    return FinishProdNumb / (ReqNumb * ColNumb);
                }
            }
            /// <summary>
            /// 按照排程标准，达成的生产时间。
            /// </summary>
            public double StdFinishTimes
            {
                get
                {
                    double _p = Performance;
                    if (_p <= 0)
                        return 0;
                    else if (_p < PerformanceRatio)
                        return StdWorkTime * _p;
                    else
                        return StdWorkTime;
                }
            }
        ...
        */
        ...
        private List<Plan_GridItem> Plan_LoadFinishedRate(DateTime NowTime, int classtype)
        {
            ...
            Plan_LoadDataSource(NowTime, classtype);
            MyRecord.Say("3.2 计算达成率和加载表格...\r\n                       设定各个条件。");
            return CalculatePerformence(NowTime, classtype);
        }
        private List<Plan_GridItem> CalculatePerformence(DateTime NowTime, int classtype)
        {
            MyRecord.Say("3.2.计算达成率和加载表格...");
            MyRecord.Say("设定各个条件。");
            DateTime fTimeBegin = DateTime.MinValue, fTimeEnd = DateTime.MinValue;
            ...
            var vPrintProduceSource = from a in Plan_Lists
                                      ...
                                      select a;
            List<Plan_Item> _LocalPringSource = vPrintProduceSource.ToList();
            List<ProcessSumItemBkp> _FinishNumbBkp1 = new List<ProcessSumItemBkp>();
            MyRecord.Say("逐笔工单计算印刷排程完工数....  ");
            foreach (var item in _LocalPringSource)
            {
                var vpmt = from a in _LocalPringSource
                           where a.RdsNo == item.RdsNo && a.MachineCode == item.MachineCode
                ...
                if (item.AutoDuplexPrint && item.ProdSide == 2 && item.Side == 0)
                {
                    //大于正面下机时间的最近一个反面的下机时间。
                    var vAutoPrint = from a in _LocalPringSource
                                     where a.MachineCode == item.MachineCode && a.DepartmentID == item.DepartmentID && a.ProcessCode == item.ProcessCode && a.PartID == item.PartID &&
                                           a.ProduceRdsNo == item.ProduceRdsNo && a.AutoDuplexPrint && a.ProdSide == 2 && a.Side == 1 &&
                                           a.Edd > item.Edd
                                     select a.Edd;
                    if (vAutoPrint.IsNotEmptySet()) item.Edd = vAutoPrint.Min();
                }
                ...
                //此处计算完工数
                var vf = from a in Plan_FinishLists
                         where a.ProcessCode == "2000" && a.MachineCode == item.MachineCode && a.ProduceRdsNo == item.ProduceRdsNo && a.PartID == item.PartID && a.Side == item.Side
                             && a.InputDate >= fTimeBegin && a.InputDate <= fTimeEnd
                             && a.StartTime <= a.InputDate //完工单不能提前输入，开始时间必须小于输入时间
                             //&& a.StartTime >= item.Bdd.AddHours(-1) && a.StartTime <= item.Edd.AddHours(1)  //以排程一单起始结束时间前后30分钟计算，不在范围内为无效。
                             && a.StartTime >= StartTime.AddMinutes(-1) && a.EndTime <= EndTime.AddMinutes(10)  //以排程整体开始结束时间计算，不在当日排程范围内无效。
                         select a;
                double finishProdNumb = vf.Sum(x => x.FinishProdNumb), finishNumb = vf.Sum(x => x.FinishNumb);

                if (item.StaticMeasurement == 1)  //如果良品不良品同时计算达成率
                {
                    finishProdNumb = vf.Sum(x => (x.FinishProdNumb + x.RejectProdNumb + x.LossProdNumb));
                    finishNumb = vf.Sum(x => x.FinishNumb + x.RejectNumb + x.LossNumb);
                }
                ...
                if (vppt != null && vppt.Count() > 1)  //在排程中大于一行的时候。
                {
                    var vbkp = from a in _FinishNumbBkp1
                               where a.RdsNo == item.RdsNo && a.ProduceRdsNo == item.ProduceRdsNo && a.PartID == item.PartID && a.MachineCode == item.MachineCode && a.Side == item.Side
                               select a;
                    if (vbkp != null && vbkp.Count() > 0)
                    {
                        ProcessSumItemBkp psp = vbkp.FirstOrDefault();
                        if ((finishProdNumb - psp.FinishProdNumb) <= (item.ReqNumb * item.ColNumb) || psp.SumTime >= vppt.Count())
                        {
                            finishProdNumb = finishProdNumb - psp.FinishProdNumb;
                            finishNumb = finishNumb - psp.FinishSheetNumb;
                        }
                ...
                if ((item.ReqNumb <= 500) && (finishNumb > (item.ReqNumb * 1.10)))   //印刷小于500的，不可以超出110%
                {
                    item.FinishProdNumb = item.ReqNumb * item.ColNumb * 1.10;
                    item.FinishSheetNumb = item.ReqNumb * 1.10;
                }
                ...
                item.Type = ((item.Bdd - item.PlanBegin).TotalMinutes > -10 && (item.Edd - item.PlanEnd).TotalMinutes < 10) ? LCStr("正常") : LCStr("超出");
                foreach (var iv in vf) { iv.iCalItem = true; iv.Memo = string.Empty; iv.GridStyleName = "GreenBackColor"; }
                var vfSet = from a in Plan_FinishLists
                            where a.ProcessCode == "2000" && a.iCalItem == false // && a.MachineCode == item.MachineCode && a.ProduceRdsNo == item.ProduceRdsNo && a.PartID == item.PartID && a.Side == item.Side
                            orderby a.StartTime, a.EndTime
                            select a;
                foreach (var iv in vfSet)
                {
                    if (iv.InputDate < fTimeBegin)
                    {
                        iv.Memo = LCStr(string.Format("完工单输入时间早于{0:yy/MM/dd HH:mm}", fTimeBegin));
                        iv.GridStyleName = "GrayFont";
                    }
                    else if (iv.InputDate > fTimeEnd)
                    {
                        iv.Memo = LCStr(string.Format("完工单输入时间大于{0:yy/MM/dd HH:mm}", fTimeEnd));
                        iv.GridStyleName = "GrayFont";
                    }
                    else if (iv.StartTime > iv.InputDate) //输入时间早于开始时间
                    {
                        iv.Memo = LCStr("完工单上机时间大于完工单输入时间");
                        iv.GridStyleName = "YellowBackColor";
                    }
                    else if (iv.StartTime > EndTime)
                    {
                        iv.Memo = LCStr(string.Format("完工单上机时间大于排程结束时间{0:yy/MM/dd HH:mm}", EndTime));
                        iv.GridStyleName = "GrayFont";
                    }
                    else if (iv.EndTime < StartTime)
                    {
                        iv.Memo = LCStr(string.Format("完工单下机时间早于排程开始时间{0:yy/MM/dd HH:mm}", StartTime));
                        iv.GridStyleName = "GrayFont";
                    }
                    else if (iv.StartTime < StartTime)
                    {
                        iv.Memo = LCStr(string.Format("完工单上机时间早于排程开始时间{0:yy/MM/dd HH:mm}", StartTime));
                        iv.GridStyleName = "BlueFont";
                    }
                    else if (iv.EndTime > EndTime)
                    {
                        iv.Memo = LCStr(string.Format("完工单下机时间大于排程结束时间{0:yy/MM/dd HH:mm}", EndTime));
                        iv.GridStyleName = "BlueFont";
                    }
                    //else if (iv.StartTime < item.Bdd.AddMinutes(-30))
                    //{
                    //    iv.Memo = LCStr(string.Format("完工单上机时间早于排程上机时间{0:yy/MM/dd HH:mm} 30分钟", item.Bdd));
                    //    iv.GridStyleName = "BlueFont";
                    //}
                    //else if (iv.EndTime > item.Edd.AddMinutes(30))
                    //{
                    //    iv.Memo = LCStr(string.Format("完工单下机时间大于排程下机时间{0:yy/MM/dd HH:mm} 30分钟", item.Edd));
                    //    iv.GridStyleName = "BlueFont";
                    //}
                    else if (iv.ProduceRdsNo != item.ProduceRdsNo)
                    {
                        iv.Memo = LCStr(string.Format("工单号不对，排程是：{0}", item.ProduceRdsNo));
                        iv.GridStyleName = "RedFont";
                    }
                    else if (iv.MachineCode != item.MachineCode)
                    {
                        iv.Memo = LCStr(string.Format("机台不对，排程的机台是：{0}", item.MachineName));
                        iv.GridStyleName = "RedFont";
                    }
                    else if (iv.PartID != item.PartID)
                    {
                        iv.Memo = LCStr(string.Format("部件不对，排程是：{0}", item.PartID));
                        iv.GridStyleName = "RedFont";
                    }
                    else if (iv.Side != item.Side)
                    {
                        iv.Memo = LCStr(string.Format("印刷面不对，排程是：{0}", item.AutoDuplexPrint ? "双翻" : item.Side == 0 ? "正面" : "反面"));
                        iv.GridStyleName = "RedFont";
                    }
                    else
                    {
            ...
            MyRecord.Say("计算印刷达成率....  ");
            var vPrintGrid = from a in _LocalPringSource
                             where (!(a.AutoDuplexPrint && a.ProdSide == 2 && a.Side == 1))
                             group a by new { a.RdsNo, a.DepartmentID, a.ProcessCode, a.MachineCode } into g
                             orderby g.Key.MachineCode, g.Key.RdsNo
                             select new Plan_GridItem(g, classtype);
            _GridData = vPrintGrid.ToList();


            MyRecord.Say("逐笔工单计算各个后制排程完工数。");//除了印刷，其他都是一面。
            var vProduceSource = from a in Plan_Lists
                                 where a.Side == 0 && (a.Bdd - a.PlanBegin).TotalMinutes > -5 && a.ProcessCode != "2000"
                                 select a;
            List<ProcessSumItemBkp> _FinishNumbBkp2 = new List<ProcessSumItemBkp>();
            foreach (var item in vProduceSource)
            {
                var vpmt = from a in Plan_Lists
                           where a.RdsNo == item.RdsNo && a.MachineCode == item.MachineCode
                ...
                var vf = from a in Plan_FinishLists
                         where a.InputDate >= fTimeBegin && a.InputDate <= fTimeEnd && a.ProcessCode == item.ProcessCode && a.MachineCode == item.MachineCode
                               ...
                               && a.StartTime <= a.InputDate //完工单不能提前输入，开始时间必须小于输入时间
                             //&& a.StartTime >= item.Bdd.AddMinutes(-30) && a.EndTime <= item.Edd.AddMinutes(30)  //以排程一单起始结束时间前后30分钟计算，不在范围内为无效。
                               && a.StartTime >= StartTime.AddMinutes(-1) && a.EndTime <= EndTime.AddMinutes(10)  //以排程整体开始结束时间计算，不在当日排程范围内无效。
                         select a;
                // d.PlanEnd.AddMinutes(15)
                double finishProdNumb = vf.Sum(x => x.FinishProdNumb), finishNumb = vf.Sum(x => x.FinishNumb);
                if (item.StaticMeasurement == 1)  //如果良品不良品同时计算达成率
                {
                    finishProdNumb = vf.Sum(x => (x.FinishProdNumb + x.RejectProdNumb + x.LossProdNumb));
                ...
                if (item.ProcessCode != "9035")
                {
                    ...
                    if (vppt != null && vppt.Count() > 1)
                    {
                        var vbkp = from a in _FinishNumbBkp2
                                   where a.RdsNo == item.RdsNo && a.ProduceRdsNo == item.ProduceRdsNo && a.PartID == item.PartID && a.MachineCode == item.MachineCode
                                   select a;
                        if (vbkp != null && vbkp.Count() > 0)
                        {
                ...
                foreach (var iv in vf) { iv.iCalItem = true; iv.Memo = string.Empty; iv.GridStyleName = "GreenBackColor"; }
                var vfSet = from a in Plan_FinishLists
                            where a.iCalItem == false && a.MachineCode == item.MachineCode && a.ProduceRdsNo == item.ProduceRdsNo && a.PartID == item.PartID
                            select a;
                foreach (var iv in vfSet)
                {
                    if (iv.InputDate < fTimeBegin)
        ...
        void SendProdPlanEmailLoader(DateTime nowTime)
        {
            MyRecord.Say("定时计算完工数和领料数线程创建.......");
            Thread t = new Thread(new ParameterizedThreadStart(SendProdPlanEmail));
            t.IsBackground = true;
            MyRecord.Say("定时计算完工数和领料数线程已经启动。");
            t.Start(nowTime);
        }

        void SendProdPlanEmail(object thisTime)
        {
            DateTime NowTime = (DateTime)thisTime;
            try
            {
                ...
                if (NowTime > DateTime.MinValue && classtype > 0)
                {
                    ...
                    if (vGridDataSource.Count() > 0)
                    {
                        foreach (var item in vGridDataSource)
                        {
                            ...
                            string PlanFinishStatusColor = "green";
                            if (DateTime.Now > item.EDD)
                            {
                                if (item.Y2 >= 0.98d)
                                    PlanFinishStatusColor = "green";
                                else
                                    PlanFinishStatusColor = "red";
                            }
                            else
                            {
                                if (item.Y2 >= 0.98d)
                                    ...
                                    PlanFinishStatusColor = item.PlanFinishStatus;
                            }
