            ...
            public Plan_GridItem(IGrouping<object, Plan_Item> g)
            {
                ...
                if (g.Count() > 0)
                {
                    ...
                    if (vd != null && vd.Count() > 0)
                    {
                        BDD = vd.Min(x => x.Bdd);
                        EDD = vd.Max(x => x.Edd);
                        double LossTime = vd.Sum(x => x.AdjustTime + x.PrepTime);
                        PlanActivationTime = (EDD - BDD).TotalHours - (LossTime / 60.0);
...
Insert Into [_PMC_ProdPlan_OEE](PlanRdsNo,PlanType,PlanBegin,PlanEnd,DepartmentID,DepartmetHrNumb,ProcessCode,MachineCode,PlanNumb,FinishNumb,PlanProdNumb,FinishProdNumb,RejectNumb,RejectProdNumb,WorkLevel,PlanActivationTime)
                              Values(@PlanRdsNo,@PlanType,@PlanBegin,@PlanEnd,@DepartmentID,@DepartmetHrNumb,@ProcessCode,@MachineCode,@PlanNumb,@FinishNumb,@PlanProdNumb,@FinishProdNumb,@RejectNumb,@RejectProdNumb,@WorkLevel,@PlanActivationTime)";
                        MyData.MyDataParameter[] amps = new MyData.MyDataParameter[]
                        {
                            new MyData.MyDataParameter("@PlanRdsNo",item.RdsNo),
                            new MyData.MyDataParameter("@PlanType",classtype, MyData.MyDataParameter.MyDataType.Int ),
